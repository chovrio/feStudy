...
如果在项目里面引入了其它项目，包含了另一个 git 仓库(存在.git 文件夹)需要把.git 文件夹删除。不然 git 不会管理那个项目只会上传一个同名空文件。如果情况已经发生了，不要切换分支!!!不然之前那个项目就会被删除了，只能重新拉取(那么之前做了的改动也没了)。大部分这种项目里面嵌套原始项目的情况都是读源码且个人开发者的情况，最简单的处理方式就是删除项目的.git(自己的和拉取的项目的都要删除)，然后重新绑定下远程仓库，重新 push 一遍(但是要 force 强制覆盖)。比较温和点的方式就是对拉取下来的项目做个备份，然后删除掉项目，推上去再重新添加回去。
